...
class ALFlowLyzer(object):
    ...
    def __load_reference_headers(self):
        reference_file = getattr(self.__config, 'output_columns_reference_file', None)
        if not reference_file:
            return None
        try:
            with open(reference_file, newline='') as ref_csv:
                reader = csv.reader(ref_csv)
                return next(reader, None)
        except Exception as error:
            print(f">> Error loading output_columns_reference_file ({reference_file}): {error}. "
                  "Generated headers will be used.")
            return None
        ...
        while True:
            if len(self.__flows) >= self.__config.feature_extractor_min_flows:
                ...
                pool.starmap_async(feature_extractor.execute,
                        ...
                        self.__config.features_allow_list)])
                del temp_flows
    ...
    def writer(self):
        ...
        forced_headers = self.__load_reference_headers()
        write_headers = True
        while True:
            if len(self.__data) > self.__config.writer_min_rows:
                ...
                if write_headers:
                    writer.write(file_address, self.__data, header_writing_mode,
                            only_headers=True, headers=forced_headers)
                    write_headers = False
                temp_data = []
                with self.__data_lock:
                    temp_data.extend(self.__data)
                    self.__data[:] = []
                writer.write(file_address, temp_data, data_writing_mode, headers=forced_headers)
            ...
            with self.__feature_extractor_watchdog_lock:
                if self.__extractor_thread_finish.get():
                    if write_headers:
                        writer.write(file_address, self.__data, header_writing_mode,
                                only_headers=True, headers=forced_headers)
                    writer.write(file_address, self.__data, data_writing_mode, headers=forced_headers)
                    return
